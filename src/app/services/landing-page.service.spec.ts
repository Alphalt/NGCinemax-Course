...
describe('LandingPageService', () => {
  ...
  it('Debe llamar el método getSocialNetworks() y retornar las redes sociales', () => {
    ...
    const req = httpMock.expectOne('https://cinemax-fbf6e.firebaseio.com/RedesSociales.json');
    expect(req.request.method).toBe('GET');
    req.flush(mockResponseRedes);
  });

  it('Debe llamar el método getMovies() y retornar las películas', () => {

    service.getMovies().subscribe((movies)=>{
      let estrenos = movies.filter(movie => !movie.cartelera);
      let carteleras = movies.filter(movie => movie.cartelera);
      expect(estrenos.length).toBeGreaterThanOrEqual(6);
      expect(carteleras.length).toBeGreaterThanOrEqual(6);
    });

    const req = httpMock.expectOne('https://cinemax-fbf6e.firebaseio.com/Peliculas.json');
    expect(req.request.method).toBe('GET');
